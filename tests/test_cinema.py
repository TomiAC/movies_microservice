...
def test_read_all_cinemas(client, cinema_fixture):
    initial_response = client.get("/cinemas/")
    initial_data = initial_response.json()
    initial_total = initial_data.get("total", 0)

    client.post("/cinemas/", json={"name": "Cinema Two", "location": "789 Pine St", "number": 3})
...
def test_update_cinema(client, cinema_fixture):
    cinema_id = cinema_fixture["id"]
    update_data = {"name": "Updated Cinema City", "number": 10}
    response = client.put(f"/cinemas/{cinema_id}", json=update_data)
    assert response.status_code == 200
    data = response.json()
...
def test_delete_cinema(client, cinema_fixture):
    cinema_id = cinema_fixture["id"]
...
def test_update_cinema_not_found(client):
    response = client.put("/cinemas/non_existent_id", json={"name": "New Name"})
    assert response.status_code == 404

def test_delete_cinema_not_found(client):
    response = client.delete("/cinemas/non_existent_id")
    assert response.status_code == 404

def test_create_cinema_missing_field(client):
    response = client.post("/cinemas/", json={"name": "Missing Location"})
    assert response.status_code == 422 # Unprocessable Entity

def test_get_cinemas_pagination(client):
    initial_response = client.get("/cinemas/")
    initial_data = initial_response.json()
    initial_total = initial_data.get("total", 0)
    ...
    # Create 15 cinemas
    for i in range(15):
        client.post("/cinemas/", json={"name": f"Cinema {i}", "location": f"Location {i}", "number": i})
