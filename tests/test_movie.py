def test_create_movie(client, director_fixture, genre_fixture):
...
def test_update_movie(client, movie_fixture):
    movie_id = movie_fixture["id"]
    response = client.put(f"/movies/{movie_id}", json={"title": "Updated Movie"})
    assert response.status_code == 200
    data = response.json()
    assert data["title"] == "Updated Movie"

def test_update_movie_not_found(client):
    response = client.put("/movies/non_existent_id", json={"title": "Updated Movie"})
    assert response.status_code == 404

def test_delete_movie(client, movie_fixture):
    movie_id = movie_fixture["id"]
...
def test_create_movie_director_not_found(client, genre_fixture):
    genre_id = genre_fixture["id"]
    response = client.post("/movies/", json={"title": "Test Movie", "year": 2022, "rating": 5, "description": "Test description", "language": "English", "duration": 150, "trailer": "https://example.com/trailer", "image": "https://example.com/image", "director": "non_existent_director", "genres": [genre_id]})
    assert response.status_code == 400

def test_create_movie_genre_not_found(client, director_fixture):
    director_id = director_fixture["id"]
    response = client.post("/movies/", json={"title": "Test Movie", "year": 2022, "rating": 5, "description": "Test description", "language": "English", "duration": 150, "trailer": "https://example.com/trailer", "image": "https://example.com/image", "director": director_id, "genres": ["non_existent_genre"]})
    assert response.status_code == 400

def test_update_movie_genre_not_found(client, movie_fixture):
    movie_id = movie_fixture["id"]
    response = client.put(f"/movies/{movie_id}", json={"genres": ["non_existent_genre"]})
    assert response.status_code == 404