...
async def get_movie_endpoint(movie_id: str, db: Session = Depends(get_db)):
    searched_movie = get_movie(movie_id, db)
...
async def get_movie_by_title_endpoint(title: str, db: Session = Depends(get_db)):
    searched_movie = get_movie_by_title(title, db)
    if not searched_movie:
        raise HTTPException(status_code=404, detail="Movie not found")
    return searched_movie
...
async def get_list_of_movies_by_title_like_endpoint(name: str, db: Session = Depends(get_db)):
    return get_list_of_movies_by_title_like(name, db)

@movie_router.get("/genre/{genre_id}", response_model=List[MovieRead])
async def get_movies_by_genre_endpoint(genre_id: str, db: Session = Depends(get_db)):
    genre = get_genre(genre_id, db)
    if not genre:
...
async def get_movies_endpoint(page: int = 1, size: int = 10, db: Session = Depends(get_db)):
...
async def update_movie_endpoint(movie_id: str, movie: MovieUpdate, db: Session = Depends(get_db), role: UserRole = Depends(RoleRequired([UserRole.ADMIN, UserRole.STAFF]))):
    ...
    if not existing_movie:
        raise HTTPException(status_code=404, detail="Movie not found")
    updated_movie = update_movie(movie_id, movie, db)
