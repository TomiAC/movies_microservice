...
def create_auditorium_endpoint(auditorium: AuditoriumCreate, db: Session = Depends(get_db), role: UserRole = Depends(RoleRequired([UserRole.ADMIN]))):
...
def update_auditorium_endpoint(auditorium_id: str, auditorium_update: AuditoriumUpdate, db: Session = Depends(get_db), role: UserRole = Depends(RoleRequired([UserRole.ADMIN]))):
...
def delete_auditorium_endpoint(auditorium_id: str, db: Session = Depends(get_db), role: UserRole = Depends(RoleRequired([UserRole.ADMIN]))):
